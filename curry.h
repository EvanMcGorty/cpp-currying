...
//A concept for curried functions that take particular parameters
//void can be used to indicate a unit parameter, aka an empty () or invocation with zero arguments
//This is not a general purpose curry concept, and will only work with functions curried using 'curry'
...
template<typename t, typename return_t, typename...args_t>
concept curried = is_curried_v<t, return_t, args_t...>;
...
//Functions with no parameters are treated as if they take a unit type, and must be manually empty-invoked
template<typename callable_t>
constexpr auto curry(callable_t&& callable)
{
